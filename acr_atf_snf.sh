...
acr_ed -create -field atf_snf.FTcp_pair.tcp_pay_len_last -arg i64              -write -comment "tcp pay len for previous frame"
...
acr_ed -create -field atf_snf.FMcb.decode_on_store             -arg bool             -write -comment "call kafka2 decode on store/live"
...
acr -merge  -write <<EOF
acr.delete dmmeta.field  field:c_tamand.atf_snf.kapi
...
# acr -merge  -write <<EOF
#  acr.delete dmmeta.floadtuples  field:command.atf_snf.in 
# EOF
# acr -merge -write <<EOF
...
# acr_ed -create -finput -ssimfile:omdb.omnode -indexed -target:atf_snf -write -comment "include Omnode from ssimfile"
