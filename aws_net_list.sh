...
for v in items:
    ...
    if name.startswith(prefix): print(v.get("vpcId",""), v.get("state",""), v.get("cidrBlock",""), name)
'
...
for s in items:
    ...
    if not isinstance(tags,list): tags=[tags]
    name=next((t["value"] for t in tags if t.get("key")=="Name"),"")
    if name.startswith(prefix): print(s.get("subnetId",""), s.get("state",""), s.get("cidrBlock",""), name)
'
...
for r in items:
    ...
    if not isinstance(tags,list): tags=[tags]
    name=next((t["value"] for t in tags if t.get("key")=="Name"),"")
    if name.startswith(prefix): print(r.get("routeTableId",""), r.get("vpcId",""), name)
'
...
for s in items:
    ...
    if name.startswith(prefix): print(s.get("groupId",""), name, s.get("vpcId",""))
'
...
for e in items:
    ...
    if not isinstance(tags,list): tags=[tags]
    name=next((t["value"] for t in tags if t.get("key")=="Name"),"")
    print(e.get("networkInterfaceId",""), e.get("status",""), e.get("privateIpAddress",""), name)
'
