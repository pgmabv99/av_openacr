...
bool samp_tut1a::proj_XrefMaybe(samp_tut1a::Proj &row) {
    ...
    // insert proj into index zd_proj
    if (true) { // user-defined insert condition
        zd_proj_Insert(row);
    }
    return retval;
...
bool samp_tut1a::partproj_XrefMaybe(samp_tut1a::Partproj &row) {
    bool retval = true;
    (void)row;
    samp_tut1a::Part* p_p_part = row.p_part;
    if (UNLIKELY(!p_p_part)) {
        algo_lib::ResetErrtext() << "samp_tut1a.null_ref  xref:samp_tut1a.Part.c_partproj";
        return false;
    }
    // insert partproj into index c_partproj
    if (true) { // user-defined insert condition
        bool success = c_partproj_InsertMaybe(*p_p_part, row);
        if (UNLIKELY(!success)) {
            ch_RemoveAll(algo_lib::_db.errtext);
            algo_lib::_db.errtext << "samp_tut1a.duplicate_key  xref:samp_tut1a.Part.c_partproj"; // check for duplicate key
            return false;
        }
    }
    samp_tut1a::Proj* p_p_proj = row.p_proj;
    if (UNLIKELY(!p_p_proj)) {
        algo_lib::ResetErrtext() << "samp_tut1a.null_ref  xref:samp_tut1a.Proj.zd_partproj";
        return false;
    }
    // insert partproj into index zd_partproj
    if (true) { // user-defined insert condition
        zd_partproj_Insert(*p_p_proj, row);
    }
    return retval;
}

// --- samp_tut1a.FDb.zd_part.Insert
// Insert row into linked list. If row is already in linked list, do nothing.
void samp_tut1a::zd_part_Insert(samp_tut1a::Part& row) {
    if (!zd_part_InLlistQ(row)) {
        samp_tut1a::Part* old_tail = _db.zd_part_tail;
        row.zd_part_next = NULL;
        row.zd_part_prev = old_tail;
        _db.zd_part_tail = &row;
        samp_tut1a::Part **new_row_a = &old_tail->zd_part_next;
        samp_tut1a::Part **new_row_b = &_db.zd_part_head;
        samp_tut1a::Part **new_row = old_tail ? new_row_a : new_row_b;
        *new_row = &row;
        _db.zd_part_n++;
    }
}

// --- samp_tut1a.FDb.zd_part.Remove
// Remove element from index. If element is not in index, do nothing.
void samp_tut1a::zd_part_Remove(samp_tut1a::Part& row) {
    if (zd_part_InLlistQ(row)) {
        samp_tut1a::Part* old_head       = _db.zd_part_head;
        (void)old_head; // in case it's not used
        samp_tut1a::Part* prev = row.zd_part_prev;
        samp_tut1a::Part* next = row.zd_part_next;
        // if element is first, adjust list head; otherwise, adjust previous element's next
        samp_tut1a::Part **new_next_a = &prev->zd_part_next;
        samp_tut1a::Part **new_next_b = &_db.zd_part_head;
        samp_tut1a::Part **new_next = prev ? new_next_a : new_next_b;
        *new_next = next;
        // if element is last, adjust list tail; otherwise, adjust next element's prev
        samp_tut1a::Part **new_prev_a = &next->zd_part_prev;
        samp_tut1a::Part **new_prev_b = &_db.zd_part_tail;
        samp_tut1a::Part **new_prev = next ? new_prev_a : new_prev_b;
        *new_prev = prev;
        _db.zd_part_n--;
        row.zd_part_next=(samp_tut1a::Part*)-1; // not-in-list
    }
}

// --- samp_tut1a.FDb.zd_part.RemoveAll
// Empty the index. (The rows are not deleted)
void samp_tut1a::zd_part_RemoveAll() {
    samp_tut1a::Part* row = _db.zd_part_head;
    _db.zd_part_head = NULL;
    _db.zd_part_tail = NULL;
    _db.zd_part_n = 0;
    while (row) {
        samp_tut1a::Part* row_next = row->zd_part_next;
        row->zd_part_next  = (samp_tut1a::Part*)-1;
        row->zd_part_prev  = NULL;
        row = row_next;
    }
}

// --- samp_tut1a.FDb.zd_part.RemoveFirst
// If linked list is empty, return NULL. Otherwise unlink and return pointer to first element.
samp_tut1a::Part* samp_tut1a::zd_part_RemoveFirst() {
    samp_tut1a::Part *row = NULL;
    row = _db.zd_part_head;
    if (row) {
        samp_tut1a::Part *next = row->zd_part_next;
        _db.zd_part_head = next;
        samp_tut1a::Part **new_end_a = &next->zd_part_prev;
        samp_tut1a::Part **new_end_b = &_db.zd_part_tail;
        samp_tut1a::Part **new_end = next ? new_end_a : new_end_b;
        *new_end = NULL;
        _db.zd_part_n--;
        row->zd_part_next = (samp_tut1a::Part*)-1; // mark as not-in-list
    }
    return row;
}

// --- samp_tut1a.FDb.zd_proj.Insert
// Insert row into linked list. If row is already in linked list, do nothing.
void samp_tut1a::zd_proj_Insert(samp_tut1a::Proj& row) {
    if (!zd_proj_InLlistQ(row)) {
        samp_tut1a::Proj* old_tail = _db.zd_proj_tail;
        row.zd_proj_next = NULL;
        row.zd_proj_prev = old_tail;
        _db.zd_proj_tail = &row;
        samp_tut1a::Proj **new_row_a = &old_tail->zd_proj_next;
        samp_tut1a::Proj **new_row_b = &_db.zd_proj_head;
        samp_tut1a::Proj **new_row = old_tail ? new_row_a : new_row_b;
        *new_row = &row;
        _db.zd_proj_n++;
    }
}

// --- samp_tut1a.FDb.zd_proj.Remove
// Remove element from index. If element is not in index, do nothing.
void samp_tut1a::zd_proj_Remove(samp_tut1a::Proj& row) {
    if (zd_proj_InLlistQ(row)) {
        samp_tut1a::Proj* old_head       = _db.zd_proj_head;
        (void)old_head; // in case it's not used
        samp_tut1a::Proj* prev = row.zd_proj_prev;
        samp_tut1a::Proj* next = row.zd_proj_next;
        // if element is first, adjust list head; otherwise, adjust previous element's next
        samp_tut1a::Proj **new_next_a = &prev->zd_proj_next;
        samp_tut1a::Proj **new_next_b = &_db.zd_proj_head;
        samp_tut1a::Proj **new_next = prev ? new_next_a : new_next_b;
        *new_next = next;
        // if element is last, adjust list tail; otherwise, adjust next element's prev
        samp_tut1a::Proj **new_prev_a = &next->zd_proj_prev;
        samp_tut1a::Proj **new_prev_b = &_db.zd_proj_tail;
        samp_tut1a::Proj **new_prev = next ? new_prev_a : new_prev_b;
        *new_prev = prev;
        _db.zd_proj_n--;
        row.zd_proj_next=(samp_tut1a::Proj*)-1; // not-in-list
    }
}

// --- samp_tut1a.FDb.zd_proj.RemoveAll
// Empty the index. (The rows are not deleted)
void samp_tut1a::zd_proj_RemoveAll() {
    samp_tut1a::Proj* row = _db.zd_proj_head;
    _db.zd_proj_head = NULL;
    _db.zd_proj_tail = NULL;
    _db.zd_proj_n = 0;
    while (row) {
        samp_tut1a::Proj* row_next = row->zd_proj_next;
        row->zd_proj_next  = (samp_tut1a::Proj*)-1;
        row->zd_proj_prev  = NULL;
        row = row_next;
    }
}

// --- samp_tut1a.FDb.zd_proj.RemoveFirst
// If linked list is empty, return NULL. Otherwise unlink and return pointer to first element.
samp_tut1a::Proj* samp_tut1a::zd_proj_RemoveFirst() {
    samp_tut1a::Proj *row = NULL;
    row = _db.zd_proj_head;
    if (row) {
        samp_tut1a::Proj *next = row->zd_proj_next;
        _db.zd_proj_head = next;
        samp_tut1a::Proj **new_end_a = &next->zd_proj_prev;
        samp_tut1a::Proj **new_end_b = &_db.zd_proj_tail;
        samp_tut1a::Proj **new_end = next ? new_end_a : new_end_b;
        *new_end = NULL;
        _db.zd_proj_n--;
        row->zd_proj_next = (samp_tut1a::Proj*)-1; // mark as not-in-list
    }
    return row;
}
...
void samp_tut1a::FDb_Init() {
    ...
    _db.zd_part_head = NULL; // (samp_tut1a.FDb.zd_part)
    _db.zd_part_n = 0; // (samp_tut1a.FDb.zd_part)
    _db.zd_part_tail = NULL; // (samp_tut1a.FDb.zd_part)
    _db.zd_proj_head = NULL; // (samp_tut1a.FDb.zd_proj)
    _db.zd_proj_n = 0; // (samp_tut1a.FDb.zd_proj)
    _db.zd_proj_tail = NULL; // (samp_tut1a.FDb.zd_proj)
...
// --- samp_tut1a.Part.c_partproj.Cascdel
// Delete referred-to items.
// Deleted pointed-to item.
void samp_tut1a::c_partproj_Cascdel(samp_tut1a::Part& part) {
    samp_tut1a::Partproj *ptr = part.c_partproj;
    if (ptr) {
        partproj_Delete(*ptr);
        part.c_partproj = NULL;
    }
}
...
void samp_tut1a::Part_Uninit(samp_tut1a::Part& part) {
    ...
    c_partproj_Cascdel(part); // dmmeta.cascdel:samp_tut1a.Part.c_partproj
    ind_part_Remove(row); // remove part from index ind_part
    zd_part_Remove(row); // remove part from index zd_part
}

// --- samp_tut1a.Partproj..Uninit
void samp_tut1a::Partproj_Uninit(samp_tut1a::Partproj& partproj) {
    samp_tut1a::Partproj &row = partproj; (void)row;
    samp_tut1a::Part* p_p_part = row.p_part;
    if (p_p_part)  {
        c_partproj_Remove(*p_p_part, row);// remove partproj from index c_partproj
    }
    samp_tut1a::Proj* p_p_proj = row.p_proj;
    if (p_p_proj)  {
        zd_partproj_Remove(*p_p_proj, row);// remove partproj from index zd_partproj
    }
}

// --- samp_tut1a.Proj.zd_partproj.Cascdel
// Delete all elements in the linked list.
void samp_tut1a::zd_partproj_Cascdel(samp_tut1a::Proj& proj) {
    while (samp_tut1a::Partproj *zd_partproj_first = zd_partproj_First(proj)) {
        partproj_Delete(*zd_partproj_first);
    }
}

// --- samp_tut1a.Proj.zd_partproj.Insert
// Insert row into linked list. If row is already in linked list, do nothing.
void samp_tut1a::zd_partproj_Insert(samp_tut1a::Proj& proj, samp_tut1a::Partproj& row) {
    if (!zd_partproj_InLlistQ(row)) {
        samp_tut1a::Partproj* old_tail = proj.zd_partproj_tail;
        row.zd_partproj_next = NULL;
        row.zd_partproj_prev = old_tail;
        proj.zd_partproj_tail = &row;
        samp_tut1a::Partproj **new_row_a = &old_tail->zd_partproj_next;
        samp_tut1a::Partproj **new_row_b = &proj.zd_partproj_head;
        samp_tut1a::Partproj **new_row = old_tail ? new_row_a : new_row_b;
        *new_row = &row;
        proj.zd_partproj_n++;
    }
}

// --- samp_tut1a.Proj.zd_partproj.Remove
// Remove element from index. If element is not in index, do nothing.
void samp_tut1a::zd_partproj_Remove(samp_tut1a::Proj& proj, samp_tut1a::Partproj& row) {
    if (zd_partproj_InLlistQ(row)) {
        samp_tut1a::Partproj* old_head       = proj.zd_partproj_head;
        (void)old_head; // in case it's not used
        samp_tut1a::Partproj* prev = row.zd_partproj_prev;
        samp_tut1a::Partproj* next = row.zd_partproj_next;
        // if element is first, adjust list head; otherwise, adjust previous element's next
        samp_tut1a::Partproj **new_next_a = &prev->zd_partproj_next;
        samp_tut1a::Partproj **new_next_b = &proj.zd_partproj_head;
        samp_tut1a::Partproj **new_next = prev ? new_next_a : new_next_b;
        *new_next = next;
        // if element is last, adjust list tail; otherwise, adjust next element's prev
        samp_tut1a::Partproj **new_prev_a = &next->zd_partproj_prev;
        samp_tut1a::Partproj **new_prev_b = &proj.zd_partproj_tail;
        samp_tut1a::Partproj **new_prev = next ? new_prev_a : new_prev_b;
        *new_prev = prev;
        proj.zd_partproj_n--;
        row.zd_partproj_next=(samp_tut1a::Partproj*)-1; // not-in-list
    }
}

// --- samp_tut1a.Proj.zd_partproj.RemoveAll
// Empty the index. (The rows are not deleted)
void samp_tut1a::zd_partproj_RemoveAll(samp_tut1a::Proj& proj) {
    samp_tut1a::Partproj* row = proj.zd_partproj_head;
    proj.zd_partproj_head = NULL;
    proj.zd_partproj_tail = NULL;
    proj.zd_partproj_n = 0;
    while (row) {
        samp_tut1a::Partproj* row_next = row->zd_partproj_next;
        row->zd_partproj_next  = (samp_tut1a::Partproj*)-1;
        row->zd_partproj_prev  = NULL;
        row = row_next;
    }
}

// --- samp_tut1a.Proj.zd_partproj.RemoveFirst
// If linked list is empty, return NULL. Otherwise unlink and return pointer to first element.
samp_tut1a::Partproj* samp_tut1a::zd_partproj_RemoveFirst(samp_tut1a::Proj& proj) {
    samp_tut1a::Partproj *row = NULL;
    row = proj.zd_partproj_head;
    if (row) {
        samp_tut1a::Partproj *next = row->zd_partproj_next;
        proj.zd_partproj_head = next;
        samp_tut1a::Partproj **new_end_a = &next->zd_partproj_prev;
        samp_tut1a::Partproj **new_end_b = &proj.zd_partproj_tail;
        samp_tut1a::Partproj **new_end = next ? new_end_a : new_end_b;
        *new_end = NULL;
        proj.zd_partproj_n--;
        row->zd_partproj_next = (samp_tut1a::Partproj*)-1; // mark as not-in-list
    }
    return row;
}
...
void samp_tut1a::Proj_Uninit(samp_tut1a::Proj& proj) {
    ...
    zd_partproj_Cascdel(proj); // dmmeta.cascdel:samp_tut1a.Proj.zd_partproj
    ind_proj_Remove(row); // remove proj from index ind_proj
    zd_proj_Remove(row); // remove proj from index zd_proj
}
